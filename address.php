...
{
  window.location.href='new_address.php?number=<?php echo $number; ?>';	
}
</script>
</head>
...
<div id="main" class="m0a">
  <div class="content fl">
    <div class="title">
      <h1 class="fl">收货地址</h1>
      <div class="new_address fr" onClick="new_address();">新&nbsp;增&nbsp;地&nbsp;址</div>
    </div>
    <form action="action.php?a=change_default_address" name="change_default_address" method="post">
    <input type="hidden" name="number" value="<?php echo $number; ?>">
    <ul>
    <?php
	$sql="SELECT * FROM ".PRE."address WHERE customer_id={$_SESSION['home']['id']}";
	$result = mysql_query($sql);
	$address=array();
	...
	foreach($address as $vala){
	?>
      <li>
        <input type="text" value="<?php echo $vala['address'].', '.$vala['person'].', '.$vala['phone']; ?>">
        <span><input class="radio" type="radio" name="default" <?php if($vala['default']==1){ echo 'checked'; } ?> value="<?php echo $vala['id']; ?>"><a class="edit" href="">[编辑]</a>&nbsp;&nbsp;&nbsp;&nbsp;<a class="del" href="action.php?a=del_address&id=<?php echo $vala['id']; ?>&number=<?php echo $number; ?>">[删除]</a></span>
      </li>
    <?php } ?>
    </ul>
    </form>
    <div class="save" onClick="document.change_default_address.submit();">使&nbsp;用&nbsp;地&nbsp;址</div>
  </div>
  <div class="list fr">
    <div class="title">
      <h2>账户</h2>
