...
for($i=$start;$i<=$end;$i++){
    if($i == $page){
        $num_links.="<a href='index.php?page=$i' style='magin:0 5px;color:red;font-size=20px'>$i</a>";
        //如果是当前页，后面几个没有颜色的家伙就不要了
        continue;
    }
    $num_links .="<a href='index.php?page=$i' style='margin:0 5px;'>$i</a>";
}


$str = <<<aaa
    共{$amount}页，第{$page}页,总共{$total}条数，
    <a href="index.php?page=1">首页</a>
    <a href="index.php?page={$prev}">上一页</a>
    $left_dot
    $num_links
    $right_dot
    <a href="index.php?page={$next}">上一页</a>
    <a href="index.php?page={$amount}">尾页</a>
