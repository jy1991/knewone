...
if($_GET['search']!=''){
    //如果进来说明用户点击查询了
    $search = $_GET['search'];
    $where1 = "WHERE name LIKE '%{$search}%'";
	$where2 = "and g.name LIKE '%{$search}%'";
	$url ="&search={$search}";
}
...
$num = 10;
//得到总条数 总共有多少条数据
$sql="SELECT COUNT(id) total FROM ".PRE."goods {$where1}";
...
$sql="SELECT g.id gid,g.name gname,g.cate_id,g.price,g.stock,g.status,g.heart,g.sell,g.comment,i.id iid,i.name iname,i.is_face FROM ".PRE."goods g,".PRE."image i WHERE g.id=i.goods_id and i.is_face=1 {$where2} ORDER BY gid ASC limit $offset,$num ";
...
for($i=0;$i<count($goodslist);$i++){
    ...
    if($result && mysql_num_rows($result)>0){
        ...
        $goodslist[$i]['cate_name']=$row['name'];
    }
}
// 产生数字链接
$start = $page -2;
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  ...
  <tr>
    ...
    <tr>
     ...
     <th align="center" valign="middle" class="borderright">收藏数</th>
     <th align="center" valign="middle" class="borderright">销量</th>
     <th align="center" valign="middle" class="borderright">评论数目</th>
 ...
 ?>   
    ...
    <td align="center" valign="middle" class="borderright borderbottom"><?php echo $val['cate_name']?></td>
    ...
    <td align="center" valign="middle" class="borderright borderbottom"><?php echo $val['status']==1?'<a href="action.php?a=status&id='.$val['gid'].'&status=0">有货速抢</a>':'<a href="action.php?a=status&id='.$val['gid'].'&status=1">卖完下架</a>'?></td>
    <td align="center" valign="middle" class="borderright borderbottom"><?php echo $val['heart']?></td>
    <td align="center" valign="middle" class="borderright borderbottom"><?php echo $val['sell']?></td>
    <td align="center" valign="middle" class="borderright borderbottom"><?php echo $val['comment']?></td>
    <td align="center" valign="middle" class="borderbottom"><a href="image.php?gid=<?php echo $val['gid']?>&gname=<?php echo $val['gname']?>" target="mainFrame" onFocus="this.blur()" class="add">图片管理</a><span class="gray">&nbsp;|&nbsp;</span><a href="edit.php?cate_id=<?php echo $val['cate_id']?>&gid=<?php echo $val['gid']?>" target="mainFrame" onFocus="this.blur()" class="add">编辑</a><span class="gray">&nbsp;|&nbsp;</span><a href="action.php?a=del&gid=<?php echo $val['gid'] ?>" target="mainFrame" onClick="del_confirm();" onFocus="this.blur()" class="add">删除</a></td>
  </tr>
  <?php $i++;  }?>
  </table></td>
   </tr>
