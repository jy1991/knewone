...
if($_GET['search']!=''){
    //如果进来说明用户点击查询了
    $search = $_GET['search'];
    $where = "WHERE goods_id LIKE '%{$search}%'";
	$url ="&search={$search}";
}
...
$num = 10;
//得到总条数 总共有多少条数据
$sql="SELECT COUNT(id) total FROM ".PRE."comment {$where}";
...
$sql="SELECT * FROM ".PRE."comment {$where} ORDER BY addtime DESC limit $offset,$num ";
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valoign="top">您的位置：评价管理</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <tr>
     <th align="center" valoign="middle" class="borderright">编号</th>     <th align="center" valoign="middle" class="borderright">评价ID</th>
     <th align="center" valoign="middle" class="borderright">商品名</th>
     <th align="center" valoign="middle" class="borderright">用户名</th>     <th align="center" valoign="middle" class="borderright">内容</th>
     <th align="center" valoign="middle" class="borderright">评星</th>
     <th align="center" valoign="middle" class="borderright">订单编号</th>
     <th align="center" valoign="middle" class="borderright">生成时间</th>
     <th align="center" valoign="middle" class="borderright">状态</th>
    ...
    <td align="center" valoign="middle" class="borderright borderbottom"><?php echo $valc['id']?></td>
    <td align="center" valoign="middle" class="borderright borderbottom"><?php 
	$sql="SELECT name FROM ".PRE."goods WHERE id=".$valc['goods_id'];
	$result = mysql_query($sql);
	$name='';
	if($result && mysql_num_rows($result)>0){
	...
	?></td>
    <td align="center" valoign="middle" class="borderright borderbottom"><?php 
	$sql="SELECT name FROM ".PRE."customer WHERE id=".$valc['customer_id'];
	$result = mysql_query($sql);
	$name='';
	if($result && mysql_num_rows($result)>0){
	  $name=mysql_fetch_assoc($result);
	...
	echo $name['name'];
	?></td>
    <td align="center" valoign="middle" class="borderright borderbottom" ><?php echo substr($valc['content'],0,24)?>...</td>
    <td align="center" valoign="middle" class="borderright borderbottom"><?php echo $valc['star']?></td>
    <td align="center" valoign="middle" class="borderright borderbottom"><?php echo $valc['order_number']?></td> 
    <td align="center" valoign="middle" class="borderright borderbottom">
    <?php 
	...
	echo $time;
	?></td>
    <td align="center" valign="middle" class="borderright borderbottom"><?php if($valc['display']==0){ echo '正常'; }else{ echo '屏蔽'; }?></td>
    <td align="center" valoign="middle" class="borderbottom"><a href="review.php?id=<?php echo $valc['id']; ?>" target="mainFrame" onFocus="this.blur()" class="add">浏览评价</a><span class="gray">&nbsp;|&nbsp;</span><a href="edit.php?order_id=<?php echo $valo['id']?>" target="mainFrame" onFocus="this.blur()" class="add">回复</a><span class="gray">&nbsp;|&nbsp;</span><a href="action.php?a=shield&id=<?php echo $valc['id']; ?>&display=<?php echo $valc['display']; ?>" target="mainFrame" onFocus="this.blur()" class="add"><?php if($valc['display']==0){ echo '屏蔽'; }if($valc['display']==1){ echo '正常'; }?></a></td>
  </tr>
  <?php $i++;  }?>
  </table></td>
   </tr>
